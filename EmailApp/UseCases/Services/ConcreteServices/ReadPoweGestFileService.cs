...
{
    public class ReadPoweGestFileService : IService
    {
        //RUTA DE GUARDAR ARCHIVOS
        const string ConfigDefault = "CorreosPlantillas/";
        ...
        private int delayBetweenSends = 10;

        public ReadPoweGestFileService(IServiceScopeFactory scopeFactory)
        {
            this.scopeFactory = scopeFactory;
        }

        public ReadPoweGestFileService() { }
        ...
        /// <summary>
        /// Lee el fichero de datos de mensajes de PowerGest y envia un correo si existe el fichero
        /// </summary>
        ...
        public async Task<object> Execute(object action, object[] info)
        {
            try
            {
                using (var scope = scopeFactory.CreateScope())
                {
                    ...
                    // Obtener todas las dependencias desde el scope
                    var generarRespuestas = provider.GetRequiredService<IGenerarRespuestas>();
                    var readEmail = provider.GetRequiredService<ReadEmailService>();
                    var readPowerGest = provider.GetRequiredService<ReadPoweGestFileService>();
                    var sendEmail = provider.GetRequiredService<SendEmailService>();
                    var gesInter = provider.GetRequiredService<GesInter>();

                    // Crear instancia manualmente (si no lo registraste como singleton)
                    var serviceManager = new ServiceManager(repositoryManager, generarRespuestas, readEmail, readPowerGest, sendEmail, context, scopeFactory, gesInter);

                    //Necesito saber la ruta donde estaára el fichero (al mismo nivel que donde se ejecute maui)
                    string ruta = AppBase.GetDirBase(); //Need to checck si esto coge correctamente la ruta cuando se realiza el ejecutable

                    //SendFile("D:\\NET\\EmailApp\\EmailMAUIHugo\\EmailApp\\Mail.pwx", repositoryManager, serviceManager, context);
                    SendFile(ruta, repositoryManager, serviceManager, context);
                }
            ...
            {
                WebLog.LogError(ex, "No se puedo ejecutar el servicio ReadPowerGestFile");
                return null;
            }
        }
        ...
        //public string BaseDir = @"D:\NET\EmailApp\BackUpsHugo\EmailAppBackUps\PRUEBA\EmailMauiHUGO\EmailApp\CorreosPlantillas"; //PRUEBAS
        ...
        public async Task<int> SendFile(string fileName, RepositoryManager repositoryManager, ServiceManager serviceManager, Context context)
        {
            try
            {
                ...
                else
                {
                    foreach (object email in emails)
                    {
                            ...
                            SendMail(mail, repositoryManager, serviceManager, context);

#else

                            await SendNetMail((object[])email, repositoryManager);
        ...
        public bool SendMail(MailDef mail, RepositoryManager repositoryManager, ServiceManager serviceManager, Context context)
        {
            bool resul = true;

            string dirbasestring = mail.DirBase;
            // var txt = mail.Text;
            ...
            Usuario? myAppUser = repositoryManager.UsuarioRepository.GetAll().Where(u => u.Email == mail.User).FirstOrDefault();

            //Si el usuario que viene no existe, lo creo
            if (myAppUser == null)
            {
                //Creo esto por defecto
                Conf.User.Email = mail.User;
                Conf.User.Password = mail.Pass;
                Conf.User.ImapConexion = mail.Host;
                Conf.User.SmtpConexion = mail.Host;
                Conf.User.SmtpPort = 587;
                Conf.User.ImapPort = 993;
                Conf.User.AccessToken = "NoAccessToken";

                Plan miPlan = new Plan(PlanType.Gratuito);
                context.AddAsync(miPlan);
                context.SaveChanges();
                
                Usuario toEnter = Usuario.CreateUsuario(Conf.User.Email, Conf.User.Password, "User", miPlan);
                repositoryManager.UsuarioRepository.AddAsync(toEnter);

                Conf.User.UserId = toEnter.UsuarioId;

                //Guardo su configuracion
                BaseConfig.SaveConfig();

            }
            else
            {
                //Conf.User.UserId = myAppUser.UsuarioId;
                Conf.User.UserId = myAppUser.UsuarioId;
                BaseConfig.SaveConfig();//Creamos la ID y guardamos toda la config

                if (string.IsNullOrEmpty(Conf.User.Email))
                {
                    BaseConfig.LoadConfig(); //La cargamos (si no está cargada)
                }
            }

            serviceManager.AddService("SendEmailService", new object[] {message}, delayBetweenSends, true);
        ...
        public MimeMessage CreateMail(MailDef mail)
        {
            ...
            // message.To.Add(MailboxAddress.Parse("hugo.infoser@gmail.com"));
            ...
            */
            ...
            // Procesar texto a html para el cuerpo
            string html = null;
        ...
        public async Task<bool> SendNetMail(object[] email, RepositoryManager repositoryManager)
        {
            bool resul = true;
            ...
            try
            {
                ...
                mail.To.Add("hugo.infoser@gmail.com");
                ...
                if (resul)
                {
                    //Busca al usuario que está intentando enviar el mensaje
                    //Usuario? myAppUser = repositoryManager.UsuarioRepository.GetAll().Where(u => u.Email == user).FirstOrDefault();
                    Usuario? myAppUser = repositoryManager.UsuarioRepository.GetAll().Where(u => u.Email == "hugo.infoser@gmail.com").FirstOrDefault();

                    //Si el usuario que viene no existe, lo creo
                    if (myAppUser == null)
                    {
                        //Creo esto por defecto
                        Conf.User.Email = "hugo.infoser@gmail.com";//user
                        Conf.User.Password = "bbsm ubpw rbjv prkn";//pass;
                        Conf.User.ImapConexion = "imap.gmail.com";
                        Conf.User.SmtpConexion = "smtp.gmail.com";
                        Conf.User.SmtpPort = port;
                        Conf.User.ImapPort = 993;
                        Conf.User.AccessToken = "NoAccessToken";

                        Plan miPlan = new Plan(PlanType.Gratuito);
                        Usuario toEnter = Usuario.CreateUsuario(user, pass, "User", miPlan);
                        await repositoryManager.UsuarioRepository.AddAsync(toEnter);
                    ...
                    //CAMBIAR AQUI 
                    Respuesta res = Respuesta.CreateRespuesta(null, "hugo.infoser@gmail.com"/*mail.From.ToString()*/, "hugo.infoser@gmail.com", mail.Subject, null, mail.Body, DateTime.Now);
                    await repositoryManager.RespuestaRepository.AddAsync(res);
                    res.ChangeFechaEnviado(DateTime.Now);

                    res.ChangeEsIA(false);
                    ...
                    //    foreach (ImageDef img in Images)
                    //    {
                    //        Adjunto ad = Adjunto.CreateAdjunto(res, img.Path);
                    //        repositoryManager.AdjuntoRepository.AddAsync(ad);
                    //        adjuntos.Add(ad);
                    //    }

                    //    res.SetAdjuntos(adjuntos);
                    //}

                    //repositoryManager.RespuestaRepository.Update(res);

                    MimeMessage miMimeMessage = ConvertToMimeMessage(mail);
                    ...
                    try
                    {
                        using (var smtpClient = new MailKit.Net.Smtp.SmtpClient())
                        {
                            ...
                            await smtpClient.AuthenticateAsync(Conf.User.Email, Conf.User.Password);
                            await SendEmailMaui(smtpClient, res, miMimeMessage, repositoryManager);
        ...
        public static MimeMessage ConvertToMimeMessage(MailMessage netMail, List<(string ContentId, string FilePath, string MimeType)> embeddedImages = null)
        {
            ...
            message.From.Add(MailboxAddress.Parse("hugo.infoser@gmail.com"));

            // To
            foreach (var to in netMail.To)
                message.To.Add(MailboxAddress.Parse(to.Address));

            // CC
            foreach (var cc in netMail.CC)
                message.Cc.Add(MailboxAddress.Parse(cc.Address));

            // BCC
            foreach (var bcc in netMail.Bcc)
                message.Bcc.Add(MailboxAddress.Parse(bcc.Address));

            // Subject
            message.Subject = netMail.Subject ?? "Sin Asunto";

            // HEADERS
            foreach (var key in netMail.Headers.AllKeys)
                message.Headers.Add(key, netMail.Headers[key]);

            // CONFIRMACION DE LECTURA (equivalente)
            if (netMail.DeliveryNotificationOptions.HasFlag(DeliveryNotificationOptions.OnSuccess))
            {
                message.Headers.Add("Disposition-Notification-To", netMail.From.Address);
            }

            // CUERPO
            var plainTextPart = new TextPart("plain") { Text = netMail.Body ?? "" };
            TextPart htmlPart = null;

            if (netMail.IsBodyHtml)
            {
                htmlPart = new TextPart("html") { Text = netMail.Body ?? "" };
            }

            MimeEntity body = plainTextPart;
            ...
            if (htmlPart != null)
            {
                if (embeddedImages != null && embeddedImages.Count > 0)
                {
                    ...
                    {
                        var imagePart = new MimePart(img.MimeType)
                        {
                            Content = new MimeContent(File.OpenRead(img.FilePath)),
                    ...
                    alternative.Add(plainTextPart);
                    alternative.Add(related);
                    body = alternative;
                }
                else
                {
                    // Solo texto y HTML sin imágenes
                    var alternative = new MultipartAlternative();
                    alternative.Add(plainTextPart);
            ...
            // Si hay adjuntos, envolver en multipart/mixed
            if (netMail.Attachments.Count > 0)
            {
                var mixed = new Multipart("mixed") { body };

                foreach (Attachment a in netMail.Attachments)
                {
        ...
        /// <summary>
        /// METODO DE MAUIA PARA ENVIAR CORREOS 
        /// </summary>
        /// <param name="smtServer"></param>
        /// <param name="respuesta"></param>
        /// <param name="repositoryManager"></param>
        /// <returns></returns>
        private async Task<bool> SendEmailMaui(MailKit.Net.Smtp.SmtpClient smtpClient, Respuesta respuesta, MimeMessage miMimeMessage, RepositoryManager repositoryManager)
        {
            if (respuesta == null)
            {
                return false;
            ...
            try
            {
                miMimeMessage.MessageId = respuesta.MensajeId;

                if (respuesta.CorreoId <= 0 || respuesta.CorreoId == null) //Significa que la respuesta NO tiene un correo asociado (es creada de 0 por el user) --> cada una tiene un id unico
                {
                ...
                try
                {
                    await smtpClient.SendAsync(miMimeMessage);
                }
                catch (Exception ex) { }
        ...
        private string FindImage(string image)
        {
            ...
            {
                string directPath = "C:\\Users\\programacion3\\Desktop\\PruebaImagen.png";//Path.Combine(BaseDir, image); //Funciona bien si encuentra la imagen
